...
def block_reconstruction(model: QuantModel, fp_model: QuantModel, block: BaseQuantBlock, fp_block: BaseQuantBlock,
                        cali_data: torch.Tensor, l_mask=torch.Tensor,batch_size: int = 32, iters: int = 20000, weight: float = 0.01, 
    ...
    cached_inps = get_init(model, block, cali_data,l_mask,batch_size=batch_size, 
                                        input_prob=True, keep_gpu=keep_gpu)
    cached_outs, cached_output, cur_syms = get_dc_fp_init(fp_model, fp_block, cali_data, l_mask,batch_size=batch_size, 
