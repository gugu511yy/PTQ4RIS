...
from .quant_layer import QuantModule, StraightThrough, UniformAffineWeightQuantizer,UniformAffineActQuantizer
...
class QuantModel(nn.Module):
    ...
    def quant_module_refactor(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}):
        ...
        for name, child_module in module.named_children():
            ...
            elif isinstance(child_module, (nn.Conv2d, nn.Linear))and module.__class__.__name__ == 'SimpleDecoding':
                if name !='conv1_1':
                    setattr(module, name, QuantModule(child_module, weight_quant_params, act_quant_params))
                    prev_quantmodule = getattr(module, name)

            elif isinstance(child_module, (nn.ReLU, nn.ReLU6))and module.__class__.__name__ == 'SimpleDecoding':
    ...
    def quant_module_refactor_wo_fuse(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}):
        ...
        for name, child_module in module.named_children():
            if type(child_module) in specials:
                setattr(module, name, specials[type(child_module)](child_module, weight_quant_params, act_quant_params))
            # elif isinstance(child_module, (nn.Conv2d, nn.Conv1d, nn.Linear)):
    ...
    def forward(self, image,sentence,l_mask):
        output_quantmodel= self.model(image, sentence, l_mask) 
        return output_quantmodel

    def set_first_last_layer_to_8bit(self):
        w_list, a_list = [], []
        for module in self.model.modules():
            if isinstance(module, (UniformAffineWeightQuantizer,UniformAffineActQuantizer)):
