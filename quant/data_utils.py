...
def save_dc_fp_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
                    l_mask=torch.Tensor,
                    batch_size: int = 10, keep_gpu: bool = True,
    ...
    for i in trange(int(cali_data.size(0) / batch_size)):
        ...
        if input_prob:
            cur_out, out_fp, cur_sym = get_inp_out(cali_data[i * batch_size:(i + 1) * batch_size],l_mask[i * batch_size:(i + 1) * batch_size])
            cached_batches.append((cur_out.cpu(), out_fp.cpu(), cur_sym.cpu()))
        else:
            cur_out, out_fp = get_inp_out(cali_data[i * batch_size:(i + 1) * batch_size],l_mask[i * batch_size:(i + 1) * batch_size])
...
def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
    ...
    for i in range(int(cali_data.size(0) / batch_size)):
        cur_inp = get_inp_out(cali_data[i * batch_size:(i + 1) * batch_size],l_mask[i * batch_size:(i + 1) * batch_size])
