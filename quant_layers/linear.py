...
class MinMaxQuantLinear(nn.Linear):
    ...
    @torch.jit.export
    def extra_repr(self):
        return 'w_bit={},a_bit={}'.format(self.w_bit,self.a_bit)
...
class PTQSLQuantLinear(MinMaxQuantLinear):
    ...
    @torch.jit.export
    def extra_repr(self):
        return 'w_bit={},a_bit={},init_layerwise={}'.format(self.w_bit,self.a_bit,self.init_layerwise)
...
class PTQSLBatchingQuantLinear(PTQSLQuantLinear):
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0,self.calib_size,self.calib_batch_size):
            ...
            if self.init_layerwise:
                ...
                a_interval_=((x_.view(*x_.shape[:-1],self.n_a,self.crb_acts).abs().amax(list(range(len(x_.shape)-1))+[-1],keepdim=False))/(self.a_qmax-0.5)).unsqueeze(-1) # [[1.4818]]
    ...
    def _get_pearson_w(self, tensor_raw, tensor_sim):
        ...
        similarity = torch.cosine_similarity(tensor_raw-tensor_raw_mean, tensor_sim-tensor_sim_mean, dim=1)
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        ...
        for h in range(self.n_H):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                raw_out_expanded = torch.cat(torch.chunk(raw_out_expanded.unsqueeze(-2), chunks=self.n_V, dim=-1), dim=-2)
