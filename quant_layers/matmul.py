...
class MinMaxQuantMatMul(nn.Module):
    ...
    @torch.jit.export
    def extra_repr(self):
        return 'A_bit={},B_bit={}'.format(self.A_bit,self.B_bit)
    ...
    def _search_best_A_interval(self, A, B, A_interval_candidates):
        ...
        for v, h in product(range(self.n_V_A), range(self.n_H_A)):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                A_sim = A_sim.view(p_ed-p_st,-1,A.shape[1]+self.pad_groups_A,A.shape[2]+self.pad_rows_A,A.shape[3]+self.pad_cols_A)
                A_sim = A_sim[:,:,:A.shape[1],:A.shape[2],:A.shape[3]]
                out_sim = A_sim @ B_sim
                similarity = self._get_similarity(self.raw_out, out_sim, self.metric)
                similarity = similarity.mean([1,3])
                similarities.append(similarity)
            # calculate best similarity for this block
            similarities = torch.cat(similarities, 0)
            similarities = F.pad(similarities, [0,self.pad_groups_A]).view(self.eq_n,self.n_G_A,self.crb_groups_A).mean(-1)
...
# self._initialize_calib_parameters()，self._initialize_intervals()
