...
def wrap_modules_in_bert(net,cfg):

    wrapped_modules={}
    module_dict={}
    module_types = {"query":"qlinear_classifier","key":"qlinear_classifier" ,"value":"qlinear_classifier", 'matmul1':"qmatmul_qk", 'matmul2':"qmatmul_scorev","dense":"qlinear_classifier"}

    it=[(name,m) for name,m in net.named_modules() if 'text_encoder' in name]
...
def wrap_modules_in_net(net,cfg):
    ...
    it=[(name,m) for name,m in net.named_modules() if 'backbone' in name]
    for name,m in it:
        module_dict[name]=m
        idx=name.rfind('.')
        if idx==-1:
...
# def wrap_modules_in_PWAM(net,cfg):
#
#     wrapped_modules={}
#     module_dict={}
#     module_types = {'matmul1':"qmatmul_qk", 'matmul2':"qmatmul_scorev","reduction":"qlinear_classifier"}
#     it=[(name,m) for name,m in net.named_modules() if 'fusion' in name or 'res_gate' in name or 'downsample' in name]
#     for name,m in it:
#         module_dict[name]=m
#         idx=name.rfind('.')
#         if idx==-1:
